...
const MemeBubble = () => {
  ...
  useEffect(() => {
    const loadImages = async () => {
      const loadedImages = await Promise.all(testData.map((e) => loadImage(e)));
  ...
  return (
    <>
      ...
      <div style={{position: "absolute", top: topGap-4}} className={styles.timerCompleted}/>
      <div className={styles.selectorContainer} style={selectorStyle}>
        <div className={`${styles.selector} ${duration==="1H" && styles.selected}`} onClick={() => setDuration("1H")}>
          1H
        </div>
        <div className={`${styles.selector} ${duration==="6H" && styles.selected}`} onClick={() => setDuration("6H")}>
          6H
        </div>
        <div className={`${styles.selector} ${duration==="1D" && styles.selected}`} onClick={() => setDuration("1D")}>
          1D
        </div>
        <div className={`${styles.selector} ${duration==="7D" && styles.selected}`} onClick={() => setDuration("7D")}>
          7D
        </div>
      </div>
      <Canvas style={canvasStyle} topGap={topGap} data={data} />
